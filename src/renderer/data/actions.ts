...
export async function restoreFromBackup(state: State, backupFile: string): Promise<Object> {
  const fs = getFilesystem();
  const data = JSON.parse(await fs.readFileText(backupFile));
  const nextState = {
...
export async function cleanBackups(config: Config) {
  ...
  for (let backup of backups) {
    try {
      await fs.deleteFile(saveDir + path.sep + backup.url);
    } catch (e) {
      console.error(e);
    }
  }
...
export function cacheImageBlob(path: string, blob: Blob, source: LibrarySource, config: Config): string {
  ...
  const fs = getFilesystem();
  fs.fileExists(filePath).then((exists) => {
    if (!exists) {
      return blob.arrayBuffer().then((buffer) => fs.writeFile(filePath, buffer));
    }
    return null;
  }).then(() => {
...
export function cacheImage(state: State, i: HTMLImageElement | HTMLVideoElement) {
  if (state.config.caching.enabled) {
    ...
    if (fileType != ST.local && i.src.startsWith("http")) {
      ...
      const fs = getFilesystem();
      const ensureCacheDir = () => fs.makeDirectory(cachePath).catch(() => {});
      const downloadImage = () => {
        ensureCacheDir().then(() => fs.fileExists(filePath)).then((exists) => {
          if (exists) {
            return null;
          }
          return wretch(i.src)
            ...
            .then((buffer: ArrayBuffer) => fs.writeFile(filePath, buffer));
        }).then(() => rememberPath(filePath)).catch((e) => console.error(e));
      };
      ...
      if (maxSize == 0) {
        ...
        const checkFolderSize = () => {
          ...
          fs.getDirectorySize(cachePath).then((size) => {
...
export function printMemoryReport() {
  ...
  function logCount(x: any) {
    console.log(format(x[0]), format(x[1]), format((x[1].size / (1000.0*1000)).toFixed(2)), "MB");
  }
  const memory = (performance as any).memory;
  if (memory) {
    logB(["jsHeapUsed", memory.usedJSHeapSize]);
    logB(["jsHeapTotal", memory.totalJSHeapSize]);
  }
  console.log("\n");
  console.log('------');
}
...
export function blacklistFile(state: State, sourceURL: string, fileToBlacklist: string): Object {
  ...
  if (fileToBlacklist != null) {
    ...
    const fs = getFilesystem();
    fs.fileExists(cachePath).then((exists) => {
      if (exists) {
        fs.deleteFile(cachePath).catch((e) => {
          console.error(e);
        });
      }
    }).catch((e) => {
...
export function addSource(state: State, scene: Scene, type: string, ...args: any[]): Object {
  ...
  switch (type) {
    ...
    case AF.directory:
      pickDirectory().then(dResult => {
        console.log("[import] AF.directory: got", dResult.filePaths?.length, "files");
        if (dResult.canceled || !dResult.filePaths.length) return;
        useStore.setState((currentState: any) => {
          const dupNote = (n: number | undefined) => n ? { ...systemSnack(currentState, `${n} duplicate item${n === 1 ? "" : "s"} skipped (already in this batch)`, SS.info) } : {};
          if (scene != null) {
            let addedCount = 0;
            const newScenes = currentState.scenes.map((s: Scene) => {
              if (s.id === scene.id) {
                ...
                addSources(newSources, dResult.filePaths, currentState.library);
                addedCount = newSources.length - before;
                console.log("[import] AF.directory: added", addedCount, "sources to scene", scene.id);
                return { ...s, sources: newSources };
              }
              return s;
            });
            if (addedCount === 0) {
              console.warn("[import] 0 files added to scene", scene.id);
              return { ...systemSnack(currentState, "No files were added. They may already be in the scene/library, or the import failed (low memory). Try Settings → Storage → Erase All, or import fewer files at once.", SS.warning) };
            }
            return { scenes: newScenes, ...dupNote(dResult.skippedDuplicates) };
          } else {
            const newLibrary = [...currentState.library];
            const before = newLibrary.length;
            addSources(newLibrary, dResult.filePaths, currentState.library);
            const added = newLibrary.length - before;
            console.log("[import] AF.directory: added", added, "sources to library");
            if (added === 0) {
              console.warn("[import] 0 files added to library");
              return { ...systemSnack(currentState, "No files were added. They may already be in the library, or the import failed (low memory). Try Settings → Storage → Erase All, or import fewer files at once.", SS.warning) };
            }
            return { library: newLibrary, ...dupNote(dResult.skippedDuplicates) };
          }
        });
      }).catch(e => {
        console.error("addSource AF.directory: error", e);
      });
      return;

    case AF.videos:
      pickFiles({filters: [{name:'All Files (*.*)', extensions: ['*']}, {name: 'Video files', extensions: ['mp4', 'mkv', 'webm', 'ogv', 'mov']}, {name: 'Playlist files', extensions: ['asx', 'm3u8', 'pls', 'xspf']}], properties: ['openFile', 'multiSelections']}).then(vResult => {
        ...
        console.log("[import] AF.videos: picked", vResult.filePaths.length, "→", filtered.length, "videos");
        useStore.setState((currentState: any) => {
          const dupNote = (n: number | undefined) => n ? { ...systemSnack(currentState, `${n} duplicate item${n === 1 ? "" : "s"} skipped (already in this batch)`, SS.info) } : {};
          if (scene != null) {
            let addedCount = 0;
            const newScenes = currentState.scenes.map((s: Scene) => {
              if (s.id === scene.id) {
                const newSources = [...s.sources];
                const before = newSources.length;
                addSources(newSources, filtered, currentState.library);
                addedCount = newSources.length - before;
                return { ...s, sources: newSources };
              }
              return s;
            ...
            if (addedCount === 0) {
              console.warn("[import] AF.videos: 0 added to scene", scene.id);
              return { ...systemSnack(currentState, "No videos were added. They may already be here, or the import failed (low memory). Try Settings → Storage → Erase All, or import fewer files at once.", SS.warning) };
            }
            return { scenes: newScenes, ...dupNote(vResult.skippedDuplicates) };
          } else {
            const newLibrary = [...currentState.library];
            const before = newLibrary.length;
            addSources(newLibrary, filtered, currentState.library);
            const added = newLibrary.length - before;
            console.log("[import] AF.videos: added", added, "to library");
            if (added === 0) {
              console.warn("[import] AF.videos: 0 added to library");
              return { ...systemSnack(currentState, "No videos were added. They may already be in the library, or the import failed (low memory). Try Settings → Storage → Erase All, or import fewer files at once.", SS.warning) };
            }
            return { library: newLibrary, ...dupNote(vResult.skippedDuplicates) };
          }
        });
      });
      return;
      ...
      case AF.videoDir:
        pickFiles({filters: [{name:'All Files (*.*)', extensions: ['*']}], properties: ['openDirectory', 'multiSelections']}).then(async vdResult => {
          if (vdResult.canceled || !vdResult.filePaths.length) return;
          // Recursively walk any picked directories through the Filesystem
          // service (desktop parity: Electron recurses selected folders).
          let rvResult: Array<string> = [];
          const fs = getFilesystem();
          for (let p of vdResult.filePaths) {
            try {
              const stat = await fs.stat(p);
              if (stat.isDirectory) {
                rvResult = rvResult.concat(await getFilesRecursively(p));
              } else {
                rvResult.push(p);
              }
            } catch {
        ...
        console.log("[import] AF.videoDir: collected", rvResult.length, "videos");
        useStore.setState((currentState: any) => {
          if (scene != null) {
            let addedCount = 0;
            const newScenes = currentState.scenes.map((s: Scene) => {
              if (s.id === scene.id) {
                const newSources = [...s.sources];
                const before = newSources.length;
                addSources(newSources, rvResult, currentState.library);
                addedCount = newSources.length - before;
                return { ...s, sources: newSources };
              }
              return s;
            ...
            if (addedCount === 0) {
              console.warn("[import] AF.videoDir: 0 added to scene", scene.id);
              return { ...systemSnack(currentState, "No videos were added. They may already be here, or the import failed (low memory). Try Settings → Storage → Erase All, or import fewer files at once.", SS.warning) };
            }
            return { scenes: newScenes };
          } else {
            const newLibrary = [...currentState.library];
            const before = newLibrary.length;
            addSources(newLibrary, rvResult, currentState.library);
            const added = newLibrary.length - before;
            console.log("[import] AF.videoDir: added", added, "to library");
            if (added === 0) {
              console.warn("[import] AF.videoDir: 0 added to library");
              return { ...systemSnack(currentState, "No videos were added. They may already be in the library, or the import failed (low memory). Try Settings → Storage → Erase All, or import fewer files at once.", SS.warning) };
            }
...
function getImportURLs(importURL: string, rootDir?: string): string[]  {
  if (importURL.includes("sources=")) {
    ...
    for (let u = 0; u < importURLs.length; u++) {
      ...
      if (importURLs.includes(fullPath) || importURLs[u] === path.sep || importURLs[u] === "") {
...
export function exportScene(state: State, scene: Scene): Object {
  ...
  downloadTextFile(fileName, sceneExport);
  return {};
}
...
export function exportLibrary(state: State): Object {
  ...
  downloadTextFile(fileName, libraryExport);
  return {};
}
...
export function markOffline(getState: () => State, setState: Function) {
  ...
  const offlineLoop = () => {
    ...
    if (state.progressMode == PR.cancel) {
      ...
      const exists = syncPathExists(actionSource.url);
...
/**
 * Portable binary-header dimension reader for local images (JPEG/PNG/GIF/
 * WebP/BMP). Uses the Filesystem service instead of Node `fs`.
 */
function readImageDimensions(data: ArrayBuffer): { width: number; height: number } | null {
  try {
    const buf = new Uint8Array(data.slice(0, 1024));
    ...
    if (String.fromCharCode(buf[1], buf[2], buf[3]) === 'PNG') {
      return {
    ...
    if (head.substring(0, 4) === 'RIFF' &&
      String.fromCharCode(buf[8], buf[9], buf[10], buf[11]) === 'WEBP') {
      const vp8 = String.fromCharCode(buf[12], buf[13], buf[14], buf[15]);
      if (vp8 === 'VP8X') {
        return {
          width: (((buf[24] | (buf[25] << 8)) & 0x3FFF)) + 1,
          height: (((buf[26] | (buf[27] << 8)) & 0x3FFF)) + 1,
        };
      }
      if (vp8 === 'VP8 ') {
        return {
    ...
    if (buf[0] === 0x42 && buf[1] === 0x4D) {
      const width = (buf[18] | (buf[19] << 8) | (buf[20] << 16) | (buf[21] << 24)) >>> 0;
      const heightRaw = (buf[22] | (buf[23] << 8) | (buf[24] << 16) | (buf[25] << 24)) | 0;
      return {
        width,
...
export function updateVideoMetadata(getState: () => State, setState: Function) {
  ...
  const finishSource = (offset: number) => {
    state.progressCurrent = offset + 1;
    setState({progressCurrent: state.progressCurrent});
      ...
      if (librarySource) {
        ...
        if (sourceType == ST.video) {
          ...
          video.onloadedmetadata = () => {
            ...
            video.remove();
            const vfs = getFilesystem();
            vfs.stat(librarySource.url).then((stat) => {
              librarySource.fileSize = stat.size;
            }).catch(() => {
          ...
          // Local image: read bytes through the Filesystem service and parse
          // dimension headers directly (no decode needed).
          const vfs = getFilesystem();
          Promise.all([
            vfs.readFile(librarySource.url).catch(() => new ArrayBuffer(0)),
            vfs.stat(librarySource.url).catch((): null => null),
          ]).then(([data, stat]) => {
            if (data) {
              const dims = readImageDimensions(data as ArrayBuffer);
              if (dims && dims.width > 0 && dims.height > 0) {
                librarySource.resolution = Math.min(dims.width, dims.height);
              }
            }
            librarySource.fileSize = stat && stat.size != null ? stat.size : -1;
            finishSource(offset);
          }).catch(() => {
            finishSource(offset);
          });
  ...
  if (!state.progressMode && actionableLibrary.length) {
    state.progressMode = PR.mediaAnalysis;
    state.progressCurrent = 0;
    state.progressTotal = actionableLibrary.length;
    setState({
      progressMode: state.progressMode,
