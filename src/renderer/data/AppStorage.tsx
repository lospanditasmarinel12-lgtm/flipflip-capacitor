import {getBackups, removeDuplicatesBy} from "./utils";
...
import {loadAppState, writeAppState, archiveSaveFile} from "../services/storage";
...
export default class AppStorage {
  ...
  async save(state: any, callback?: Function) {
    try {
...
async function checkAutoClean(state: any): Promise<boolean> {
  ...
  if (Date.now() - epoch > (86400000 * state.config.generalSettings.autoBackupDays)) {
    await cleanBackups(state.config);
    return true;
  }
  return false;
}
