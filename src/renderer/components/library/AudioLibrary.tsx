  ...
  render() {
    ...
    return (
      <Box sx={{ display: 'flex' }}>
        ...
        <Box component="main" sx={(theme) => ({ display: 'flex', flexGrow: 1, flexDirection: 'column', height: 'var(--app-height, 100vh)', backgroundColor: theme.palette.background.default })}>
          <Box sx={(theme) => ({ backgroundColor: theme.palette.primary.main, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '0 8px', minHeight: 'calc(64px + var(--safe-area-inset-top, env(safe-area-inset-top, 0px)))' })} />
    ...
    switch (type) {
      ...
      case AF.audios:
        (async () => {
          ...
          if (aResult.length < pickedCount) {
            console.warn("[AudioLibrary] dropped", pickedCount - aResult.length, "picked file(s): not recognized as audio");
          }
