...
import {basename} from "../services/path";
import {openExternal} from "../services/links";
import {deleteAppState} from "../services/storage";
...
export default class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error: Error, info: React.ErrorInfo) {
    // Log the full component stack so device-side failures are readable in the
    // native console (console.error always passes through the logging gate).
    console.error('[ErrorBoundary] ' + (error.name || 'Error') + ': ' + error.message);
  ...
  onFinishRestore() {
    this.onCloseDialog();
    try {
      this.props.onRestore(saveDir + "/" + this.state.backup.url);
      this.clearError();
    } catch (e) {
      console.error(e);
    }
  ...
  onSubmitIssue() {
    ...
    openExternal("https://github.com/ififfy/flipflip/issues/new?title=" + title + "&body=" + body);
  }
}
