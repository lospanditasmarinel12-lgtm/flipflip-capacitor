...
import {preventSleep, allowSleep} from "../../services/sleep";
import {clearAppCache} from "../../services/cache";
import {isFullscreen, exitFullscreen} from "../../services/window";
...
export default class Player extends React.Component {
  ...
  _hapticConnectedNotified = false;
  ...
  render() {
    ...
    if (this.props.tutorial != null && !isCapacitor()) {
      rootStyle = {
        ...rootStyle,
        pointerEvents: 'none',
      }
    ...
    return (
      <div style={rootStyle}>
        ...
        <div style={playerStyle}>
          ...
          {(this.props.config.displaySettings.audioAlert || this.props.allTags) &&
  ...
  componentDidMount() {
    ...
    if (!this._hapticsInitialized && this.props.scene.hapticsEnabled) {
      ...
      HapticService.getInstance().init(
        (devices) => this.onHapticDevices(devices),
  ...
  onHapticDevices(devices: any[]) {
    this.setState({ hapticDevices: devices });
    if (devices && devices.length > 0 && !this._hapticConnectedNotified) {
      this._hapticConnectedNotified = true;
      const names = devices.map((d: any) => d.name).filter((n: any) => !!n);
      if (this.props.systemMessage) {
        this.props.systemMessage(names.length > 0 ? 'Haptics connected: ' + names.join(', ') : 'Haptic device connected.');
      }
    }
  }
  ...
  shouldComponentUpdate(props: any, state: any): boolean {
    return this.props.scene !== props.scene ||
      ...
      this.state.hapticDevices !== state.hapticDevices ||
      this.state.hapticActive !== state.hapticActive;
  }
