      ...
      for (let line of data.data[0].split('\n')) {
        ...
        switch (command) {
          ...
          case "storeAudio":
            ...
            if (!file.startsWith("http") && !syncPathExists(file)) {
  ...
  getPhrase(value: string) {
    ...
    if (value == "$RANDOM_PHRASE") {
      ...
      if (this.props.currentImage) {
        // LiveShow history is a {url, source, post} record, not a DOM element.
        const source = typeof this.props.currentImage.getAttribute === "function"
          ? this.props.currentImage.getAttribute("source")
          : this.props.currentImage.source;
        const tag = getRandomListItem(this.props.getTags(source).filter((t) => t.phraseString && t.phraseString != ""));
        if (tag) {
