...
public class WebViewLocalServer {
    ...
    private WebResourceResponse handleLocalRequest(WebResourceRequest request, PathHandler handler) {
        ...
        // EXPERIMENT: serve every local-file request (including byte-range
        // requests from the media stack) as a full 200 body. A 206 + Content-Range
        // makes Chromium's media loader treat the resource as seekable and switch
        // to range-following fetches; those continuation reads fail on the WebView
        // interceptor and playback stops after the first fragment. A single
        // progressive 200 stream plays moov-at-start fMP4 (and even moov-at-end
        // files once downloaded) reliably without any range requests.
        if (isLocalFile(request.getUrl()) && resolveLocalFile(request.getUrl().getPath()) != null) {
            File localFile = resolveLocalFile(request.getUrl().getPath());
            try {
                String mimeType;
                try (InputStream sniffer = new FileInputStream(localFile)) {
                    mimeType = getMimeType(request.getUrl().getPath(), sniffer);
                }
                Map<String, String> headers = handler.buildDefaultResponseHeaders();
                headers.put("Content-Length", Long.toString(localFile.length()));
                ...
                Logger.error("Unable to serve local file " + request.getUrl().getPath(), e);
            }
        }
        ...
        if (rangeString != null) {
            // Real local files (the common case for large media) are served with
            // 64-bit byte ranges so files > 2 GB and long forward seeks work.
            File localFile = resolveLocalFile(path);
            if (localFile != null && localFile.isFile()) {
                try (InputStream sniffer = new FileInputStream(localFile)) {
                    String mimeType = getMimeType(path, sniffer);
                    return handleLocalRange(localFile, rangeString, mimeType, handler);
                } catch (Exception e) {
                    Logger.error("Unable to serve byte range for " + path, e);
                }
            }

            // Legacy path: content URIs and asset streams (no ordering guarantee).
        ...
        if (isLocalFile(request.getUrl()) || isErrorUrl(request.getUrl())) {
            // Real local files are served with Content-Length + Accept-Ranges so
            // the WebView media stack can seek via the range handler above
            // (Chromium stalls on media streams without a known length).
            File localFile = resolveLocalFile(path);
            if (localFile != null && localFile.isFile()) {
                try {
                    String mimeType;
                    try (InputStream sniffer = new FileInputStream(localFile)) {
                        mimeType = getMimeType(path, sniffer);
                    }
                    Map<String, String> headers = handler.buildDefaultResponseHeaders();
                    headers.put("Content-Length", Long.toString(localFile.length()));
                    ...
                    Logger.error("Unable to serve local file " + path, e);
                }
            }
            InputStream responseStream = new LollipopLazyInputStream(handler, request);
    ...
    /**
     * Resolve the real local {@link File} behind a virtual server path, or null
     * when the path maps to a content URI or an asset stream.
     */
    private File resolveLocalFile(String path) {
        if (path == null) return null;
        if (path.startsWith(capacitorFileStart)) {
            return new File(path.substring(capacitorFileStart.length()));
        }
        if (!isAsset && basePath != null && path.startsWith("/")) {
    ...
    /**
     * Serve a single byte range from a local file using 64-bit offsets so files
     * larger than 2 GB (and long forward seeks) respond correctly.
     */
    private WebResourceResponse handleLocalRange(File file, String rangeString, String mimeType, PathHandler handler) throws IOException {
        long total = file.length();
        long start = 0;
        long end = total - 1;
        String spec = rangeString.trim();
        if (spec.startsWith("bytes=")) {
            spec = spec.substring(6).trim();
        }
        if (spec.contains(",")) {
            spec = spec.substring(0, spec.indexOf(",")).trim(); // first range only
        }
        if (spec.startsWith("-")) {
            long count = Math.max(0, Math.min(Long.parseLong(spec.substring(1)), total));
            start = total - count;
            end = total - 1;
        ...
        if (start > total - 1) {
            ...
            return new WebResourceResponse(mimeType, handler.getEncoding(), 416, "Range Not Satisfiable", headers, null);
        }
        ...
        return new WebResourceResponse(mimeType, handler.getEncoding(), 206, "Partial Content", headers, stream);
    }
